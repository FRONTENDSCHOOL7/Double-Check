...
<br/>
- "더블책(Double Chaek)" 은 독서 커뮤니티 플랫폼으로, 책에 대한 정보 제공과 사용자 간의 리뷰 공유를 넘어서 **더블 체크** 라는 개념에 중점을 두고 있습니다. 이 플랫폼은 사용자들이 책을 읽고, 그들의 이해와 감상을 리뷰하거나 인상적인 문구를 공유함으로써, 한 번 더 생각하는 기회를 제공합니다.

- 네이버와 알라딘의 **도서 API** 를 활용하여 추천 도서를 제공하며, 이를 통해 사용자들이 자신의 해석을 '더블 체크'하고, 다른 독자들과 의견을 교환하며 독서 경험을 풍부하게 할 수 있도록 돕습니다.

- 프로젝트명 **더블책** 은 이러한 **더블 체크** 의 과정을 통해 독서가 갖는 의미와 가치를 재확인하는 데서 영감을 받았습니다.
...
- Heroku를 고려했지만 유료화 정책으로 인해 무료로 서버를 배포할 수 있는 Cloudflare를 사용하여 프록시 서버를 설정했습니다. Cloudflare를 통해 API 요청을 보내고 응답을 받아 웹 애플리케이션으로 전달함으로써 CORS 문제를 해결했습니다.
...
- 각각의 댓글에 대해 useTimeSince를 호출하는 대신, 각 댓글의 시간을 변환하는 로직을 별도의 컴포넌트로 분리하여 사용해야 합니다. 훅이 각 댓글 컴포넌트 내에서 호출되도록 구조를 수정해야 했습니다.

- Comments 컴포넌트에서 렌더링되던 댓글 아이템을 CommentItem 컴포넌트로 분리합니다.
  map 함수를 사용하여 각 CommentItem 컴포넌트를 렌더링합니다.
  useTimeSince 훅은 각 CommentItem 컴포넌트의 최상위에서 호출되도록 구현합니다. 이렇게 함으로써 훅이 각 댓글의 생성 시간에 대해 정상적으로 적용되며, 컴포넌트를 작은 단위로 분리하여 관리할 수 있게 됩니다.
